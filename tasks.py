...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-migration-conflicts runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-migration-conflicts.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_migration_conflicts")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-migration-conflicts tests")
...
def release(c, bumpsize=''):
    ...
    import dj_migration_conflicts
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_migration_conflicts.__version__))
    c.run("git push --tags")
